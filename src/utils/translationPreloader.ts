...
export const getTranslationValue = (
  ...
  try {
    ...
    if (language === 'ar' && GLOBAL_TRANSLATIONS.en[key]) {
      console.warn(`[TranslationPreloader] Missing Arabic translation for key: ${key}, falling back to English`);
...
/**
 * Directly access translations from the global store for critical paths
 * Use this when you need to ensure a translation is available synchronously
 */
export const getCriticalTranslation = (language: TranslationLanguages, key: string): string => {
  // First check if translations are available
  if (language === 'ar' && arTranslations[key]) {
    return arTranslations[key];
  } else if (language === 'en' && enTranslations[key]) {
